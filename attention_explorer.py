...
class AttentionExplorer:
    ...
    def visualize_addition(self, equation_string, save_path=None, all_tokens=False):
        ...
        def decode_full(t_id):
            char = self.itos[t_id]
            return char

        input_tk_names = [decode_full(t) for t in tokens]
        ...
        input_token_labels = [decode_full(t) for t in tokens]
        ...
        for i, pred_idx in enumerate(pred_indices):
            ...
            gt_display = decode_full(gt_idx)
            pred_display = decode_full(pred_idx_val)
        ...
        if all_tokens:
            ...
            for i in y_indices:
                if i in pred_to_label:
                    ...
                    y_labels.append(
                        f"{decode_full(tokens[i])} \u2192 {decode_full(tokens[i + 1])}"
                    )
        else:
        ...
        # Reverse Y-axis (last token at top, first at bottom)
