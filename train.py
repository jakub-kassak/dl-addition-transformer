...
class ValidationTableCallback(Callback):
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        table.add_column("Avg Err Pos", justify="right", style="yellow", width=11)
        ...
        for i, name in enumerate(val_names):
            ...
            avg_err_pos = get_m("val_avg_first_error_pos")
            loss = get_m("val_loss")
            ...
            if token_acc is not None:
                table.add_row(
                    ...
                    f"{avg_err_pos:.2f}" if avg_err_pos is not None else "N/A",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Type of position representation for the dataset.",
    )
    ...
    if not args.smoke_test and not args.validate_only:
        print("\n" + "="*60)
        print(" "*20 + "SPOT CHECK EXAMPLES")
        print("="*60)
        model.eval()
        model.to("cpu")

        # Define test cases: (n1, n2, expected_result_reversed)
        test_cases = [
            ("123", "456", "975"),
            ("52836", "74359", "591721"),
            ("99", "1", "001"),  # 100 reversed
            ("5", "7", "21"),  # 12 reversed
            ("987654321", "123456789", "0111111111"),
        ]

        for n1_str, n2_str, expected in test_cases:
            prompt_str = f"{n1_str}+{n2_str}="
            tokens = [dm.stoi[c] for c in prompt_str]

            # Create positional IDs based on dataset type
            if dm.dataset_type == "absolute":
                # Absolute: sequential [0, 1, 2, ...]
                pos = list(range(len(tokens)))
                offset = 50
                pos = [p + offset for p in pos]
            elif dm.dataset_type == "position_coupling":
                # Position coupling: digit offsets [3,2,1,0,3,2,1,0]
                L1, L2 = len(n1_str), len(n2_str)
                pos = list(range(L1, -1, -1)) + list(range(L2, -1, -1))
                offset = 50
                pos = [p + offset for p in pos]
            else:
                pos = list(range(len(tokens)))
            ...
            generated = model.generate(x_in, pos_in, max_new_tokens=max_len)

        # Decode function
            def decode(token_list):
                result = []
                for token_id in token_list:
                    char = dm.itos[token_id.item() if torch.is_tensor(token_id) else token_id]
                    result.append(char)
                return "".join(result)
    
            predicted = decode(generated[0][len(tokens):])
            ...
            print(f"  Expected:  {expected}  {is_correct}")
